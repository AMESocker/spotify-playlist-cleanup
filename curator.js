...
async function getCarPlaylistTrackCount() {
  ...
  const cleanCount = cleanId
    ? (sizes.find((p) => p.playlistId === cleanId)?.trackCount ?? 0)
    : 0;
  const explicitCount = explicitId
    ? (sizes.find((p) => p.playlistId === explicitId)?.trackCount ?? 0)
    : 0;
...
async function wouldExceedLimit(tracksToAdd) {
  ...
  if (current + tracksToAdd > MAX_PLAYLIST_SIZE) {
    console.log(
      `⚠️ Playlist would exceed ${MAX_PLAYLIST_SIZE} tracks (currently ${current}, adding ${tracksToAdd}).`,
    );
...
async function getOriginalTrackCount(artist, album) {
  ...
  try {
    ...
    if (!json.releases?.length) {
      console.log(
        `⚠️ MusicBrainz: No releases found for "${artist} - ${album}"`,
      );
      return null;
    }
    ...
    console.log(
      `📅 MusicBrainz original release: "${earliest.title}" (${earliest.date}) — ${trackCount} tracks`,
    );
    return trackCount;
  } catch (err) {
...
async function handleEditorsChoice(source) {
  ...
  if (status.nextWeek) {
    ...
    console.log(
      `Albums: ${status.nextWeek.enrichedAlbums}/${status.nextWeek.totalAlbums} ready`,
    );
  }
...
async function handleFestival(source, data) {
  ...
  const globalAdded = new Set(
    data.festivals.flatMap((f) =>
      f.added.map((a) => a.replace(/\s*\[.*?\]$/, "")),
    ),
  );
  ...
  function pickArtist() {
    ...
    for (let attempt = 0; attempt < festival.artists.length; attempt++) {
      ...
      if (globalAdded.has(name)) {
        console.log(
          `⏭️ Skipping ${name} — already added from another festival`,
        );
...
async function handleAlbum(source, data) {
  ...
  if (!albumInfo) {
    ...
    if (source.strategy === "sequential") {
      ...
      if (entry) {
        entry.Albums.shift();
        entry.AddedAlbums.push(`${pick.nextAlbum} [NOT FOUND]`);
      }
    }
  ...
  while (true) {
    const res = await spotify.getAlbumTracks(albumInfo.id, {
      limit: 50,
      offset,
    });
...
async function handleSingleTrack(source, data) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    try {
      ...
      addedCount++;
      console.log(
        `✅ Found: "${tracks[0].name}" by ${tracks[0].artists[0].name}`,
      );
    } catch (err) {
  ...
  try {
    ...
    while (true) {
      const response = await spotify.getPlaylistTracks(playlistId, {
        limit,
        offset,
        fields: "items(track(uri,name)),next",
      });
...
export async function addNextAlbum() {
  ...
  console.log(
    `🔀 Current data source: ${source.name} (strategy: ${source.strategy})`,
  );
  ...
  let result;
  if (source.strategy === "editorsChoice")
    result = await handleEditorsChoice(source);
  else if (source.strategy === "rockHall")
    result = await handleRockHall(source, data);
  else if (source.strategy === "festival")
    result = await handleFestival(source, data);
  else if (source.strategy === "artistGenre")
    result = await handleArtistGenre(
      source,
      wouldExceedLimit,
      pushHistory,
      saveData,
...
export async function fillPlaylist() {
  ...
  console.log(
    `📊 Final playlist size: ${trackCount}/${MAX_PLAYLIST_SIZE} tracks`,
  );
}
